import numpy as np

# --- Simulation Parameters ---
GRAVITY = 9.81
BALL_MASS = 1.0
TIME_STEP = 0.02  # 50 Hz simulation frequency
FORCE_MAGNITUDE = 50.0  # Max force applied by controller

# --- Pygame Visualization Parameters ---
SCREEN_WIDTH = 400
SCREEN_HEIGHT = 600
BALL_RADIUS = 20
GROUND_HEIGHT = 0

# --- Colors ---
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
GREEN = (0, 255, 0)
BLUE = (0, 0, 255)

# --- RL Training Parameters ---
PPO_TIMESTEPS = 15000000
MAX_EPISODE_STEPS = 2000

# --- Environment Settings ---
TARGET_HEIGHT = 300.0
STARTING_HEIGHT = 100.0
RANDOM_REFERENCE = False #True/False
MOVING_REFERENCE = False #True/False
MOVING_REFERENCE_TYPE = 'sine'  # 'sine' or 'sigmoid'
SINE_REFERENCE_FREQUENCY = 1
SINE_REFERENCE_AMPLITUDE = 50
SIGMOID_REFERENCE_AMPLITUDE = 50
SIGMOID_REFERENCE_PERIOD = 0.3
SIGMOID_REFERENCE_SLOPE = 10
SIGMOID_REFERENCE_SHIFT = np.pi / (2 * SIGMOID_REFERENCE_PERIOD)
PERLIN_OCTAVES = 4
PERLIN_AMPLITUDE = 4
PERLIN_FREQUENCY = 1.0

# --- Measurement Noise ---
STD_POS = 20
STD_VEL = 10

# --- EKF Parameters ---
EKF_VAR_PROC_POS = 10
EKF_VAR_PROC_VEL = 9
EKF_VAR_MEAS_POS = STD_POS**2
EKF_VAR_MEAS_VEL = STD_VEL**2

# --- MHE Parameters ---
MHE_HORIZON = 10
MHE_VAR_PROC_POS = 10
MHE_VAR_PROC_VEL = 9
MHE_VAR_MEAS_POS = STD_POS**2
MHE_VAR_MEAS_VEL = STD_VEL**2
MHE_SMALL_WEIGHT = np.eye(2)*1e-12
MHE_GROWING_HORIZON = False

# --- Standard MPC Parameters ---
STD_MPC_HORIZON = 10
STD_MPC_QH = 100
STD_MPC_QV = 20
STD_MPC_R = 0.05
STD_MPC_TERMINAL = 10
STD_MPC_DELTA_U_MAX = 15

# --- Stochastic MPC Parameters ---
STOCHASTIC_MPC_HORIZON = 10
STOCHASTIC_MPC_SAMPLES = 5
STOCHASTIC_MPC_QH = 100
STOCHASTIC_MPC_QV = 20
STOCHASTIC_MPC_R = 0.05
STOCHASTIC_MPC_TERMINAL = 10
STOCHASTIC_MPC_DELTA_U_MAX = 15
STOCHASTIC_MPC_NOISE_STD = 0.3

# --- Tube MPC Parameters ---
TUBE_MPC_HORIZON = 10
TUBE_MPC_QH = 100
TUBE_MPC_QV = 20
TUBE_MPC_R = 0.05
TUBE_MPC_TERMINAL = 10
TUBE_MPC_DELTA_U_MAX = 15
TUBE_MPC_TIGHTENING_H = 10
TUBE_MPC_TIGHTENING_V = 5
TUBE_MPC_TIGHTENING_U = 5
TUBE_MPC_LQR_SCALE = 0.01